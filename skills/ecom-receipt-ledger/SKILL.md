...
description: 手写单据电子化台账技能。把手机拍的手写采购单/销售单照片，经多模态识别转成结构化明细，做手写简写标准化映射（如「14pm改17pm」「15DD屏幕」「XR」→ 标准品名）、数量×单价自动算账与差异标红、采购/销售按天按月的日结汇总与净收益对账，并把原始单据照片作为可点击超链接与缩略图钉回每一行数据，方便随时查验。涉及「手写单据录入」「拍照记账」「存根簿电子化」「每日采购销售对账」「闭店对账」「单据照片和表格关联」「简写看不懂/对不上」这类请求时使用。
metadata:
...
```text
拍照 → 多模态识别（结构化）→ 简写标准化 → 自动算账与差异标红 → 采购/销售日结 → 凭证回链
```

**边界：OCR 由多模态模型完成，本技能的脚本不做图像识别。** 脚本负责识别之后的所有确定性工作：字段归一、简写映射、算账校验、日结汇总、凭证关联与台账导出。识别质量靠 [references/ocr-playbook.md](references/ocr-playbook.md) 里的拍照规范与提示词模板保证。
...
`--out-xlsx` 一次给六张工作表：**单据明细 / 日结 / 月结 / 异常行 / 待映射简写 / 凭证索引**。示例数据与样张见 `examples/`（`examples/photos/` 里有 8 张示例单据照片）。
...
| 等级 | 本技能场景 | AI 权限 | 人工权限 |
|---|---|---|---|
| 高 | 金额不符、无法复核、方向判不出、隔离比例 > 20% | 算差异 + 标红 + 隔离 | 回看原始单据，确认识别错还是单子本身错 |
| 中 | 简写未映射、单据未写金额、识别置信度低、凭证缺照片、疑似重复行 | 出清单与建议 | 补映射表、补照片、确认重复 |
| 低 | 日期补年份、方向写法归一、金额精度统一、日结汇总 | 直接执行 | 事后抽查 |

**不猜数据、不静默丢行。** 算不了的进隔离清单，映射不了的在待映射表里列出来，缺照片的标出来——每一笔都有交代。
...
- `data` 关键字段：`purchase` / `sales` / `net` / `net_margin`、`calc_check`（一致/小额差异/金额不符/无法复核笔数与差异合计）、`alias`（命中率与 top 未映射）、`photos`（linked/missing）、`daily`（逐日采销与净收益）。
...
| `--out-xlsx` | Excel 工作簿 | 六张工作表，表头加粗冻结首行、数字可求和；凭证索引含缩略图 |
...
- 上游：[`ecom-data-prep`](../ecom-data-prep/) 的字段归一与清洗口径；识别环节可用 `see` / `claude-vision-skill` 之类视觉能力。
